...
<div class="relative" use:clickOutside>
    <button on:click={() => open = !open} type="button" title="Toggle theme">
        {#if current === 'light'}
    ...
    {#if open}
        <div class="absolute right-0 mt-1 w-36 rounded-lg bg-white dark:bg-gray-800 shadow-lg z-50">
            {#each ['light', 'dark', 'system'] as opt}
                <button on:click={() => setTheme(opt)} class="block w-full px-4 py-2 text-sm text-left {current === opt ? 'font-bold' : ''}">{opt.charAt(0).toUpperCase() + opt.slice(1)}</button>
            {/each}
        </div>
    {/if}
</div>
