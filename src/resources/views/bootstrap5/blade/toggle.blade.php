<div x-data="{
    open: false,
    current: localStorage.getItem('{{ $storageKey }}') || '{{ $userPreference() }}',
    init() { this.apply(); window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => { if (this.current === 'system') this.apply(); }); },
    set(mode) {
        this.current = mode; localStorage.setItem('{{ $storageKey }}', mode); this.apply(); this.open = false;
        @if($persistToServer) @auth
        fetch('{{ $persistRoute }}', { method: '{{ $persistMethod }}', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content || '{{ csrf_token() }}', 'Accept': 'application/json' }, body: JSON.stringify({ {{ $persistField }}: mode }) }).catch(() => {});
        @endauth @endif
    },
    apply() { document.documentElement.classList.toggle('{{ config('darkmode.class_name', 'dark') }}', this.current === 'dark' || (this.current === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches)); }
}" class="dropdown d-inline-block" {{ $attributes ?? '' }}>
    ...
    <div x-show="open" x-cloak @click.away="open = false" class="dropdown-menu dropdown-menu-end show">
